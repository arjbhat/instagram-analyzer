// Client-side sentiment analysis using the sentiment library (fully offline)
import Sentiment from 'sentiment';
import { isMessageMeaningful } from './instagram-parser';
...
export function analyzeSentiment(text: string): SentimentResult {
  ...
  try {
    ...
    // Determine intensity based on comparative score
...
export function analyzeConversationSentiment(messages: Array<{ 
  content?: string; 
  timestamp_ms: number; 
  sender_name: string;
  ...
  const meaningfulMessages = messages.filter(msg => isMessageMeaningful({
    ...msg,
    is_geoblocked_for_viewer: msg.is_geoblocked_for_viewer || false,
    is_unsent_image_by_messenger_kid_parent: msg.is_unsent_image_by_messenger_kid_parent || false
  }));
  ...
  const sentimentResults = meaningfulMessages.map(msg => ({
    ...msg,
