...
export default function AnalyticsPage() {
  const {
    followers, following, engagement, searchData, postsViewed,
    mutualFollowers, followersOnly, followingOnly, followRatio,
    loading, error
  } = useInstagramData();
  ...
  const sortedFollowingOnly = followingOnly.sort((a, b) => b.timestamp - a.timestamp);
  const sortedFollowersOnly = followersOnly.sort((a, b) => b.timestamp - a.timestamp);
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `instagram-analytics-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  };
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <GlowingCard>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{engagement.likes.length}</div>
            <p className="text-xs text-muted-foreground">
      ...
      <Tabs defaultValue="social" className="space-y-4">
        <div className="flex flex-col gap-3">
          <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3">
            <TabsList className="grid w-full grid-cols-4 xs:w-auto xs:flex xs:space-x-1">
              <TabsTrigger value="social" className="text-xs px-2 py-1.5 xs:px-3 xs:py-2">
                <span className="hidden xs:inline">Social Network</span>
                <span className="xs:hidden">Social</span>
              </TabsTrigger>
              <TabsTrigger value="engagement" className="text-xs px-2 py-1.5 xs:px-3 xs:py-2">
                <span className="hidden xs:inline">Engagement</span>
                <span className="xs:hidden">Likes</span>
              </TabsTrigger>
              <TabsTrigger value="activity" className="text-xs px-2 py-1.5 xs:px-3 xs:py-2">
                <span className="hidden xs:inline">Activity Patterns</span>
                <span className="xs:hidden">Activity</span>
              </TabsTrigger>
              <TabsTrigger value="search" className="text-xs px-2 py-1.5 xs:px-3 xs:py-2">
                <span className="hidden xs:inline">Search Behavior</span>
                <span className="xs:hidden">Search</span>
              </TabsTrigger>
            </TabsList>
            <Button onClick={exportData} variant="outline" size="sm" className="gap-2 w-full xs:w-auto">
              <Download size={14} />
        ...
        <TabsContent value="social" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ChartCard
              ...
              description={`${followingOnlySearch ? `${filteredFollowingOnly.length} of ${followingOnly.length} accounts` : `${followingOnly.length} accounts`} not following back`}
            ...
            <ChartCard
              ...
              description={`${followersOnlySearch ? `${filteredFollowersOnly.length} of ${followersOnly.length} accounts` : `${followersOnly.length} accounts`} you don't follow back`}
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {engagement.saved.slice(0, 15).map((saved, i) => (
                    <ClickableListItem key={i} href={saved.href}>
                      <span className="text-sm truncate">{saved.title || 'Post'}</span>
                      <span className="text-xs text-muted-foreground">
                        {new Date(saved.timestamp * 1000).toLocaleDateString()}
                      </span>
                    </ClickableListItem>
                  ))}
                </div>
            ...
            {postsViewed.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {postsViewed.slice(0, 20).map((post, i) => (
                  <ClickableListItem key={i} href={`https://www.instagram.com/${post.author}`}>
        ...
        <TabsContent value="search" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {searchData.profileSearches.length > 0 && (
              <ChartCard
                ...
                data={searchData.profileSearches.map(s => ({ timestamp: s.searchTime, value: s.value })) as any}
                className="lg:col-span-2"
                groupBy="day"
              >
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {searchData.profileSearches.slice(0, 20).map((search, i) => (
                    <ClickableListItem key={i} href={`https://www.instagram.com/${search.value}`}>
                      <span className="text-sm">{search.value}</span>
                      <span className="text-xs text-muted-foreground">
                        {new Date(search.searchTime * 1000).toLocaleDateString()}
                      </span>
                    </ClickableListItem>
                  ))}
                </div>
            ...
            {searchData.wordSearches.length > 0 && (
              <GlowingCard className={searchData.profileSearches.length > 0 ? "" : "lg:col-span-3"}>
                <CardHeader>
                  <CardTitle>Search Terms</CardTitle>
                  <CardDescription>{searchData.wordSearches.length} keywords searched</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-wrap gap-2 max-h-[400px] overflow-y-auto">
                    {searchData.wordSearches.slice(0, 30).map((search, i) => (
                      <a
                        key={i}
                        href={`https://www.instagram.com/explore/search/keyword/?q=${encodeURIComponent(search.value)}`}
                        target="_blank"
          ...
          {searchData.profileSearches.length === 0 && searchData.wordSearches.length === 0 && (
